...
class Simulation:
    def __init__(self, dce_number: int = 26, spec_len: int = 64, target_size: int = 32, components: int = 2, cfg: dict = {}):
    ...
    def load_default_curve(self):
        ...
        self.chemical_dce_curve_new = torch.stack((water_dy, glu_dy, lac_dy), dim=0).to(self.device)[:, :self.dce_number]
    ...
    def generate_chemical_density_map(self, 
        ...
        ChemicalDensityMap = torch.zeros(self.components, self.dce_number, W, H)
    ...
    @torch.no_grad()
    def simulation(self, water_img: Union[np.ndarray, "torch.Tensor"], glu_img: Union[np.ndarray, "torch.Tensor"]):
        ...
        self.generate_chemical_density_map(water_image=water_imag, glu_image=glu_imag)
        self.chemical_density_map = self.chemical_density_map.to(self.device).to(self.torch_dtype)

        chemical_com = self.components
        chemical_shift = torch.tensor([2.2, 4.8], dtype=self.torch_dtype, device=self.device)[:chemical_com] - 3.03
...
class BatchSimulation(Simulation):
    def __init__(self, 
        ...
        super().__init__(dce_number=dce_number, spec_len=spec_len, target_size=target_size, cfg=cfg)
    ...
    def generate_chemical_density_map(self, 
        ...
        W, H = self.target_size, self.target_size
        ChemicalDensityMap = torch.zeros(water_image.shape[0], self.components, self.dce_number, W, H, dtype=self.torch_dtype, device=self.device)
